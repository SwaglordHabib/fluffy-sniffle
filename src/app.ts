...
// export let io = require("socket.io")(server,{serveClient:false});
...
io.sockets.on('connection', (socket: Socket) => {
    ...
    socket.on('create', async (data) => {
        ...
        try {
            ...
            await client.db(DRUNKENPOLLS).collection(GAMES).insertOne({ Pin: "" + data.Pin + "", Player: [{ Name: data.Player, Points: 0, Role: "GM", connection: socket.id }], Creation: new Date().toISOString() });

            let result = await client.db(DRUNKENPOLLS).collection(GAMES).findOne({ Pin: "" + data.Pin + "" });
    ...
    socket.on('nextPoll', async (poll) => {
        ...
        try {
            ...
            let result = await client.db(DRUNKENPOLLS).collection(DRUNKENPOLLS).aggregate([{ $sample: { size: 1 } }]).toArray();
            console.log(result);
            ...
            game = await client.db(DRUNKENPOLLS).collection(GAMES).findOne({ Pin: `${poll.Pin}` });
            ...
            switch (question.type) {
                case Type.Estimate:
                    ...
                    let newplayers = game.Player.map((player: IPlayer) => {
                        player.Points = parseInt(player.Points) + parseInt(differences.filter((d: IPlayer) => d.connection === player.connection)[0].Points).toString();
                        return player;
                    });
                    await client.db(DRUNKENPOLLS).collection(GAMES).updateOne({ Pin: poll.Pin }, { $set: { Player: newplayers } });
                    io.in(poll.Pin).emit(EMIT_NEXT, { Poll: 2, Pin: poll.Pin, Player: calculatePoints(differences) });
                    break;
    ...
    socket.on('nextState', async (data) => {
        ...
        try {
            ...
            let game = await client.db(DRUNKENPOLLS).collection(GAMES).findOne({ Pin: `${data.Pin}` });
            io.in(data.Pin).emit(EMIT_NEXT, { Poll: 0, Pin: data.Pin, Player: game.Player });
        } catch (error) {
            console.error(`${new Date()}|${error}`);
        } finally {
            await client.close();
    ...
    socket.on('vote', async (poll) => {
        ...
        try {
            await client.connect();
            console.log("Connected to Mongodb");
            polls.push({ Pin: poll.Pin, Estimate: poll.Estimate, SocketId: socket.id });
    ...
    socket.on('join', async (data) => {
        ...
        try {
            ...
            playerarray.push({ Name: data.Player, Points: 0, Role: "Player", connection: socket.id });

            await client.db(DRUNKENPOLLS).collection(GAMES).updateOne({ Pin: data.Pin }, { $set: { Player: playerarray } });
    ...
    socket.on('disconnect', async (reason) => {
        ...
        try {
            ...
            let game = await client.db(DRUNKENPOLLS).collection(GAMES).findOne({ "Player.connection": socket.id });
            let pin = game.Pin;
            let playerindex = game.Player.findIndex((x: IPlayer) => x.connection === socket.id);
            game.Player.splice(playerindex, 1);

            await client.db(DRUNKENPOLLS).collection(GAMES).updateOne({ Pin: pin }, { $set: { Player: game.Player } });

            let result = await client.db(DRUNKENPOLLS).collection(GAMES).findOne({ Pin: pin });
